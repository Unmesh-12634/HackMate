    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 lg:p-8">
            ...
            <motion.div
                ...
                className="w-full max-w-5xl h-full max-h-[85vh] bg-slate-900 border border-white/10 rounded-[2.5rem] relative z-10 shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-[1.5fr_1fr]"
            >
                {/* Left Side: Post Content */}
                <div className="flex flex-col h-full border-r border-white/5 bg-slate-900/50">
                    <div className="p-4 border-b border-white/5 flex items-center justify-between lg:hidden">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">SIGNAL_DETAILS</span>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                        ...
                        <div className="flex items-center gap-4 mb-8">
                            <Avatar className="w-14 h-14 rounded-2xl border border-white/10 shadow-2xl">
                                <AvatarImage src={post.avatar} />
                                <AvatarFallback>{post.user[0]}</AvatarFallback>
                            </Avatar>
                            <div>
                                <h3 className="text-xl font-black text-white tracking-tight">{post.user}</h3>
                                ...
                                <div className="rounded-2xl bg-black/40 border border-white/10 overflow-hidden shadow-2xl">
                                    <div className="px-6 py-3 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[11px] font-black text-slate-400 font-['JetBrains_Mono'] uppercase tracking-widest">
                            ...
                            {post.type === 'project' && post.projectDetails && (
                                <div className="rounded-[2rem] border border-blue-500/30 bg-blue-500/[0.03] shadow-2xl shadow-blue-500/5 p-8">
                                    <div className="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            <h4 className="text-xl font-black text-white uppercase tracking-tight">{post.projectDetails.name}</h4>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-3">
                                            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">TECH_STACK</div>
                                            <div className="flex flex-wrap gap-2">
                                                {post.projectDetails.techStack?.map(tech => (
                                                    <span key={tech} className="px-3 py-1 bg-slate-950 border border-white/10 rounded-lg text-xs font-bold text-slate-300">
                                                        {tech}
                                                    </span>
                                                ))}
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">OPEN_ROLES</div>
                                            <div className="flex flex-wrap gap-2">
                                    ...
                                    <Button
                                        ...
                                        className="w-full mt-8 bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-widest py-8 rounded-2xl shadow-2xl shadow-blue-600/30 border-0 transition-all text-sm"
                                    >
                        ...
                        <div className="mt-8 flex flex-wrap gap-2">
                            {post.tags.map(tag => (
                                <span key={tag} className="px-4 py-2 rounded-xl bg-slate-950 border border-white/5 text-[11px] font-black text-blue-500 uppercase tracking-widest shadow-inner">
                                    #{tag}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="p-8 border-t border-white/5 bg-slate-950/30 flex items-center justify-between">
                        <div className="flex items-center gap-8">
                            <button
                                ...
                                className={cn(
                                    ...
                                    post.isLiked ? "text-blue-500" : "text-slate-500 hover:text-white"
                                )}
                            >
                        ...
                        <button className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-slate-500 hover:text-white transition-all">
                ...
                <div className="flex flex-col h-full bg-slate-900/30 overflow-hidden relative">
                    <button
                        onClick={onClose}
                        className="absolute top-6 right-6 p-3 bg-slate-950/50 hover:bg-slate-800 rounded-2xl text-slate-400 hover:text-white transition-all z-20 border border-white/5 shadow-2xl hidden lg:block"
                    >
                        <X className="w-5 h-5" />
                    </button>
                            ...
                            comments.map(comment => (
                                <div key={comment.id} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
                                    <div className="flex gap-4 group/comment">
                                        <Avatar className="w-10 h-10 rounded-xl border border-white/10 shrink-0 shadow-lg transition-transform group-hover/comment:scale-105">
                                            <AvatarImage src={comment.author_avatar} />
                                            <AvatarFallback>{comment.author_name[0]}</AvatarFallback>
                                        </Avatar>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="text-[11px] font-extrabold text-white tracking-tight">{comment.author_name}</span>
                                                <span className="text-[8px] font-bold text-slate-600 uppercase font-['JetBrains_Mono']">
                                                    {new Date(comment.created_at).toLocaleDateString()}
                                                </span>
                                            </div>
                                            <div className="bg-slate-950/50 border border-white/[0.03] p-3.5 rounded-2xl text-[12px] font-medium text-slate-300 leading-relaxed group-hover/comment:border-blue-500/20 transition-all shadow-inner">
                                                {comment.content}
                                            </div>
                                            <button
                    ...
                    <div className="p-6 bg-slate-950/40 border-t border-white/5">
                        <AnimatePresence>
                            {replyTo && (
                                ...
                                >
                                    ...
                                    <button onClick={() => setReplyTo(null)} className="text-slate-500 hover:text-white transition-all"><X className="w-3.5 h-3.5" /></button>
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        <form onSubmit={handleCommentSubmit} className="relative group/form">
                            <textarea
                                ...
                                className="w-full h-24 bg-slate-900/50 border border-white/10 rounded-[1.5rem] p-4 text-xs font-medium text-white outline-none focus:border-blue-500/50 transition-all placeholder:text-slate-700 resize-none shadow-inner"
                            />
                            <button
                                type="submit"
                                disabled={!commentInput.trim()}
                                className="absolute bottom-3 right-3 p-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20 disabled:bg-slate-800 disabled:text-slate-600 disabled:shadow-none hover:scale-105 active:scale-95"
                            >
