...
const TacticalBeacon: React.FC<{
   ...
   <div className="p-4 rounded-xl bg-card/40 border border-border/20 hover:border-blue-500/30 transition-all group cursor-pointer relative overflow-hidden">
      ...
      <div className="flex items-start gap-4 relative z-10">
         <Avatar className="w-8 h-8 rounded-lg border border-border/30">
         ...
         <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
               ...
               <span className="text-[9px] font-bold text-muted-foreground/70 uppercase font-['Fira_Code']">{time}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
               {tags.slice(0, 2).map(tag => (
                  <span key={tag} className="px-1.5 py-0.5 rounded-sm bg-card border border-border/30 text-[8px] font-black text-muted-foreground uppercase tracking-tighter">#{tag}</span>
               ))}
            </div>
         </div>
      </div>
...
const CommunicationMatrix: React.FC<{
   ...
   onMarkAllRead
      ...
      const navigate = useNavigate();
      const [mode, setMode] = useState<"global" | "bounties">(initialMode as any === "global" ? "global" : "bounties");
      const [searchQuery, setSearchQuery] = useState("");
      const [searchResults, setSearchResults] = useState<User[]>([]);

      useEffect(() => {
         if (initialMode === "global" || initialMode === "bounties") {
...
const PostShareWidget: React.FC<{
   ...
   return (
      <div className="p-6 rounded-[2rem] bg-card/10 border border-border/15 shadow-lg">
         <div className="flex gap-4">
            <Avatar className="w-12 h-12 rounded-2xl border border-border/30 shadow-2xl">
               <AvatarImage src={user.avatar} />
               <AvatarFallback>{user.name[0]}</AvatarFallback>
            </Avatar>
            <form onSubmit={handleSubmit} className="flex-1">
               <input
                  className="w-full bg-card/60 border border-border/30 rounded-[1.25rem] py-4 px-6 text-[11px] font-medium text-foreground outline-none focus:border-indigo-500/30 transition-all placeholder:text-muted-foreground/30"
               ...
               <div className="mt-4 flex items-center justify-between">
                  <div className="flex gap-2">
                     <button type="button" className="p-2 rounded-lg hover:bg-muted/50 text-muted-foreground/70 hover:text-indigo-400 transition-all"><Plus className="w-4 h-4" /></button>
                     <button type="button" className="p-2 rounded-lg hover:bg-muted/50 text-muted-foreground/70 hover:text-indigo-400 transition-all"><Globe className="w-4 h-4" /></button>
                  </div>
...
const PostCommentSection: React.FC<{
   ...
   return (
      <div className="mt-6 pt-6 border-t border-border/15 space-y-4">
         <div className="space-y-6">
            {loading ? (
               <div className="py-2 text-center text-[9px] font-bold text-muted-foreground/50 uppercase animate-pulse font-['Fira_Code']">SCANNING_COMMS...</div>
            ) : comments.map(c => (
               <div key={c.id} className="space-y-4">
                  <div className="flex gap-3 group/comment">
                     <Avatar className="w-8 h-8 rounded-lg border border-border/30 shrink-0">
                     ...
                     <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                           ...
                           <span className="text-[8px] font-bold text-muted-foreground/50 font-['Fira_Code']">{new Date(c.created_at).toLocaleDateString()}</span>
                        </div>
                        <p className="text-[11px] font-medium text-foreground/80 leading-relaxed bg-card/50 p-3 rounded-xl border border-border/15">{c.content}</p>
                        <button
                           onClick={() => setReplyTo(c)}
                           className="mt-2 text-[9px] font-black text-muted-foreground hover:text-blue-400 uppercase tracking-widest transition-colors flex items-center gap-1.5"
                        >
                  ...
                  {c.replies && c.replies.length > 0 && (
                     <div className="ml-11 space-y-4 border-l border-border/15 pl-4">
                        {c.replies.map(r => (
                           <div key={r.id} className="flex gap-3">
                              <Avatar className="w-6 h-6 rounded-md border border-border/30 shrink-0">
                              ...
                              <div className="flex-1 min-w-0">
                                 <div className="flex items-center gap-2 mb-0.5">
                                    ...
                                    <span className="text-[7px] font-bold text-muted-foreground/50">{new Date(r.created_at).toLocaleDateString()}</span>
                                 </div>
                                 <p className="text-[10px] font-medium text-muted-foreground leading-relaxed bg-card/30 p-2 rounded-lg">{r.content}</p>
                              </div>
                           </div>
                        ))}
                     </div>
         ...
         <form onSubmit={handleSubmit} className="flex flex-col gap-2 pt-2">
            ...
            <div className="flex-1 relative">
               <input
                  className="w-full h-10 bg-card/60 border border-border/50 rounded-xl px-4 text-[11px] font-medium text-foreground outline-none focus:border-blue-500/50 transition-all placeholder:text-muted-foreground/50"
                  placeholder={replyTo ? "TRANSMIT_DATA_REPLY..." : "TRANSMIT_SIGNAL..."}
                  value={input}
                  onChange={e => setInput(e.target.value)}
               />
               <button className="absolute right-1 top-1/2 -translate-y-1/2 p-2 text-muted-foreground/70 hover:text-blue-500">
                  <Send className="w-4 h-4" />
               </button>
            </div>
         </form>
   ...
   return (
      <div className="grid grid-cols-2 gap-4 pb-2 scrollbar-hide">
         <div className="flex flex-col items-center gap-2 group cursor-pointer">
            <div className="w-full aspect-square rounded-2xl bg-card border border-border flex items-center justify-center group-hover:border-indigo-500/50 transition-all shadow-inner">
               <Plus className="w-5 h-5 text-muted-foreground/70 group-hover:text-indigo-500" />
            </div>
            <span className="text-[8px] font-black text-muted-foreground/70 uppercase tracking-widest font-['Fira_Code']">NEW_PULSE</span>
         </div>
         {teams.map(team => (
            <div
               key={team.id}
            ...
            >
               ...
               <span className="text-[8px] font-black text-foreground/80 uppercase tracking-tight truncate w-full text-center">{team.name}</span>
            </div>
         ))}
      </div>
   );
         ...
         <div className="max-w-[1440px] mx-auto md:p-8 space-y-4 md:space-y-8 relative z-10 transition-all pb-24 md:pb-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 border-b border-border/25 pb-4 md:pb-6 px-4 pt-4 md:p-0">
               ...
               <div className="flex items-center gap-3 w-full md:flex-1 md:max-w-xl mx-auto justify-between">
                     ...
                     <AnimatePresence>
                        {(searchResults.length > 0 || isSearching) && (
                           <motion.div
                              ...
                              exit={{ opacity: 0, y: -10 }}
                              className="absolute top-14 left-0 w-full bg-card border border-border rounded-xl shadow-2xl z-[60] overflow-hidden"
                           >
                                 ...
                                 {isSearching ? (
                                    ...
                                    <div className="space-y-1">
                                       {searchResults.map(res => (
                                          <div
                                             key={res.id}
                                             className="flex items-center justify-between p-3 hover:bg-muted/10 rounded-lg transition-colors group cursor-pointer"
                                             onClick={() => navigate(`/u/${res.id}`)}
                                          >
                                             <div className="flex items-center gap-3">
                                                <Avatar className="w-10 h-10 border border-border/70">
                                                   <AvatarFallback className="bg-muted text-foreground/80 font-bold">{res.name[0]}</AvatarFallback>
                                                </Avatar>
                                             ...
                                             <button
                                                className="p-2 rounded-full hover:bg-blue-500/10 text-muted-foreground hover:text-blue-500 transition-all opacity-0 group-hover:opacity-100"
                              ...
                              <div className="p-3 bg-white/[0.02] border-t border-border text-[10px] text-muted-foreground uppercase tracking-widest font-bold text-center">
                  ...
                  >
                     <MessageSquare className={cn("w-5 h-5", isChatOpen ? "text-blue-500" : "text-muted-foreground")} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_2.5fr_1fr] gap-6">
               ...
               <div className="hidden lg:flex flex-col gap-6">
                  ...
                  {user && (
                     <div className="bg-card rounded-xl border border-border overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer">
                        <div className="h-16 bg-blue-600/20 border-b border-border w-full relative">
                           <div className="absolute -bottom-10 left-1/2 -translate-x-1/2">
                              <Avatar className="w-20 h-20 rounded-xl border-4 border-background bg-card">
                        ...
                        <div className="pt-12 pb-6 px-6 text-center">
                           <h3 className="text-lg font-bold text-foreground hover:underline">{user.name}</h3>
                           <p className="text-sm font-medium text-muted-foreground mt-1">{user.rank}</p>
                           ...
                           <div className="flex flex-col gap-3 text-sm font-medium text-muted-foreground">
                              ...
                              <div className="flex justify-between items-center hover:text-foreground transition-colors">
                                 ...
                                 <span className="font-['JetBrains_Mono'] text-foreground font-semibold">{teams.length}</span>
                              </div>
                              <div
                              ...
                              >
                                 ...
                                 <span className="font-['JetBrains_Mono'] text-foreground font-semibold group-hover/stat:text-blue-400">{followerCount}</span>
                              </div>
                              <div
                              ...
                              >
                                 ...
                                 <span className="font-['JetBrains_Mono'] text-foreground font-semibold group-hover/stat:text-blue-400">{followingCount}</span>
                              </div>
                           </div>
                        </div>
                     </div>
                  ...
                  <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
                     <h3 className="text-sm font-semibold text-foreground mb-4">Lead Your Teams</h3>
                     <div className="space-y-3 relative z-10 overflow-y-auto max-h-64 scrollbar-hide">
                        {leadTeams.length === 0 ? (
                           <div className="text-center py-4 text-sm font-medium text-muted-foreground italic">No squads under your command</div>
                        ) : leadTeams.map(team => (
                           <div key={team.id} className="group/squad flex items-center justify-between p-2 rounded-lg hover:bg-muted transition-colors cursor-pointer">
                              <div className="flex items-center gap-3 overflow-hidden" onClick={() => window.location.href = `/team/${team.id}`}>
                                 <div className="w-10 h-10 rounded shadow-sm bg-muted border border-border/70 flex items-center justify-center font-['JetBrains_Mono'] text-blue-400 font-bold shrink-0">
                                    {team.name[0]}
                                 </div>
                                 <div className="flex-1 min-w-0">
                                    <h4 className="text-sm font-semibold text-foreground/90 truncate group-hover/squad:text-blue-400 transition-colors">{team.name}</h4>
                                    <p className="text-xs text-muted-foreground mt-0.5">{team.currentMembers.length} members</p>
                                 </div>
                              </div>
                              <button
                                 onClick={(e) => { e.stopPropagation(); setSelectedRecruitTeam(team); setShowRecruitModal(true); }}
                                 className="p-2 mr-1 rounded-md hover:bg-blue-500/10 text-muted-foreground hover:text-blue-500 transition-all opacity-0 group-hover/squad:opacity-100"
                     ...
                     <div className="bg-card md:rounded-xl border-y md:border border-border p-4 shadow-sm flex flex-col gap-3 relative z-10">
                        ...
                        <div className="flex items-center justify-between pt-2 px-1">
                           <div className="flex gap-1 flex-wrap">
                              <button onClick={() => setShowBroadcastModal(true)} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-blue-400 transition-colors text-sm font-semibold">
                     ...
                     <AnimatePresence mode="popLayout">
                        {filteredPosts.length > 0 ? (
                           ...
                           <motion.div
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                              className="p-12 flex flex-col items-center justify-center text-center space-y-4 bg-card rounded-2xl border border-border border-dashed"
                           >
                              <div className="w-16 h-16 rounded-3xl bg-card flex items-center justify-center text-muted-foreground/50 border border-border">
                                 <Activity className="w-8 h-8 opacity-20" />
                              </div>
                              <div className="space-y-1">
                                 <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest">No Intelligence Detected</h3>
                                 <p className="text-xs text-muted-foreground/70 font-bold uppercase tracking-tighter">Broadcast your first mission update to initialize the grid.</p>
                              </div>
                              <Button
                                 onClick={() => setShowBroadcastModal(true)}
               ...
               <div className="hidden lg:flex col-span-1 flex-col gap-6">
                  ...
                  <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
                     <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-sm font-semibold text-foreground">HackMate News</h3>
                     </div>
                     <div className="flex flex-col gap-3">
                        {[
                           ...
                           <div key={i} className="flex flex-col gap-0.5 group cursor-pointer">
                              <span className="text-sm font-semibold text-foreground/80 group-hover:text-blue-400 hover:underline transition-colors">{t.tag}</span>
                     ...
                     <div className="space-y-4">
                        {allProfiles.filter(p => p.id !== user?.id).slice(0, 4).map((op, i) => (
                           <div key={i} className="flex gap-3">
                              <Avatar className="w-10 h-10 rounded-full border border-border cursor-pointer hover:border-blue-500/50 transition-colors">
                                 <AvatarFallback>{op.name[0]}</AvatarFallback>
                              </Avatar>
                              <div className="flex flex-col flex-1 min-w-0">
                                 <h4 className="text-sm font-semibold text-foreground/90 truncate hover:text-blue-400 hover:underline cursor-pointer transition-colors">{op.name}</h4>
                                 <p className="text-xs text-muted-foreground truncate mb-1">{op.role}</p>
                                 <button
                                    ...
                                    className={cn(
                                       ...
                                       followingIds.includes(op.id) ? "border-border text-muted-foreground hover:bg-muted" : "border-border/70 text-foreground/80 hover:bg-muted hover:border-border/50"
                                    )}
                                 >
            ...
            {showBroadcastModal && (
               <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                  ...
                  <motion.div
                     ...
                     className="w-full max-w-lg bg-card border border-border/70 rounded-xl relative z-10 shadow-2xl overflow-hidden flex flex-col"
                  >
                     <div className="flex items-center justify-between p-4 border-b border-border">
                        <h3 className="text-lg font-bold text-foreground">Create a post</h3>
                        <button onClick={() => setShowBroadcastModal(false)} className="p-2 text-muted-foreground hover:text-foreground/90 hover:bg-muted rounded-full transition-colors cursor-pointer">
                           <X className="w-5 h-5" />
                        </button>
                     </div>
                     ...
                     <div className="p-4 flex items-center gap-3">
                        ...
                        <div>
                           ...
                           <p className="text-xs font-semibold text-muted-foreground">Post to anyone</p>
                        </div>
                     </div>
                     ...
                     <div className="px-4 pb-4">
                        <textarea
                           ...
                           className="w-full h-24 bg-transparent border-none text-sm text-foreground outline-none resize-none placeholder:text-muted-foreground"
                        />

                        <div className="mt-2 space-y-2">
                           <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Upgrade_Visual_Telemetry (Image URL)</label>
                           <input
                              value={newPostImageUrl}
                              onChange={e => setNewPostImageUrl(e.target.value)}
                              placeholder="https://example.com/mission_intel.png"
                              className="w-full bg-card/50 border border-border/30 rounded-xl px-4 py-2 text-xs text-foreground/80 outline-none focus:border-blue-500/30 transition-all font-medium"
                           />
                        </div>
                     </div>
                     <div className="flex items-center justify-between p-4 border-t border-border">
                              ...
                              <button
                                 ...
                                 className={cn(
                                    ...
                                    selectedTag === tag
                                       ...
                                       : "border-border/70 text-muted-foreground hover:bg-muted hover:text-foreground/90"
                                 )}
                              >
                        ...
                        <Button
                           ...
                           className="px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-muted disabled:text-muted-foreground text-foreground rounded-full text-sm font-semibold transition-colors cursor-pointer shadow-sm"
                        >
                           Broadcast
                        </Button>
                     </div>
            ...
            {isChatOpen && (
               <>
                  <motion.div
                     ...
                     className="fixed inset-0 z-[110] bg-card/20 backdrop-blur-[2px]"
                  />
                  <motion.div
                     initial={{ x: "100%" }}
                     animate={{ x: 0 }}
                     ...
                     className="fixed top-0 right-0 h-full w-full max-w-md bg-card border-l border-border/25 z-[120] shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)]"
                  >
                     <div className="flex flex-col h-full bg-card">
                        <div className="p-4 border-b border-border/25 flex items-center justify-between">
                           <h2 className="text-sm font-black uppercase tracking-widest text-foreground font-['JetBrains_Mono']">Communication_Matrix</h2>
                           <button onClick={() => setIsChatOpen(false)} className="p-2 hover:bg-muted/50 rounded-lg text-muted-foreground transition-colors">
                              <X className="w-5 h-5" />
                           </button>
                        </div>
   ...
   return (
      <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
         <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="absolute inset-0 bg-card/90 backdrop-blur-md"
         />
         <motion.div
            initial={{ scale: 0.9, opacity: 0, y: 20 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            exit={{ scale: 0.9, opacity: 0, y: 20 }}
            className="w-full max-w-2xl bg-card border border-border/40 rounded-2xl shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]"
         >
            {/* Header */}
            <div className="p-6 border-b border-border/25 flex items-center justify-between bg-white/[0.02]">
               <div className="flex items-center gap-3">
                  <div className="p-2 bg-blue-500/20 rounded-lg text-blue-400">
                     <Target className="w-5 h-5" />
                  </div>
                  <div>
                     <h3 className="text-xl font-bold text-foreground uppercase tracking-tight">Deploy Recruitment Post</h3>
                     <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest mt-0.5">Project: {team.name}</p>
                  </div>
               </div>
               <button onClick={onClose} className="p-2 hover:bg-muted/50 rounded-full text-muted-foreground transition-colors">
                  <X className="w-5 h-5" />
            ...
            <div className="p-8 overflow-y-auto space-y-8 scrollbar-hide">
               ...
               <div className="space-y-3">
                  <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-1">Mission Intel & Requirements</label>
                  <textarea
                     value={description}
                     onChange={(e) => setDescription(e.target.value)}
                     placeholder="Deploy a brief description of the mission, technical requirements, and any critical objectives..."
                     className="w-full h-32 bg-card/50 border border-border/25 rounded-xl p-4 text-sm text-foreground/90 outline-none focus:border-blue-500/50 transition-all resize-none placeholder:text-muted-foreground/70"
                  />
               </div>

               {/* Image Update */}
               <div className="space-y-3">
                  <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-1">Visual_Upgrade (Mission Image)</label>
                  <input
                     value={imageUrl}
                     onChange={(e) => setImageUrl(e.target.value)}
                     placeholder="https://example.com/mission_visual.jpg"
                     className="w-full bg-card/50 border border-border/25 rounded-xl px-4 py-3 text-sm text-foreground/90 outline-none focus:border-blue-500/50 transition-all font-medium"
                  />
               </div>

               {/* Roles Dropdown Section */}
               <div className="space-y-3">
                  <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-1">Select Open Roles</label>
                  ...
                  <div className="relative">
                     ...
                     <input
                        ...
                        className="w-full h-12 bg-card/50 border border-border/25 rounded-xl pl-11 pr-4 text-sm text-foreground/90 outline-none focus:border-blue-500/50 transition-all font-medium"
                     />
                     ...
                     <AnimatePresence>
                        {rolesQuery.trim() !== "" && (
                           <motion.div
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: -10 }}
                              className="absolute top-14 left-0 w-full bg-card border border-border/40 rounded-xl shadow-2xl z-20 max-h-48 overflow-y-auto p-2 border-t-0"
                           >
                              {filteredRoles.length === 0 ? (
                                 <div className="p-4 text-center text-xs text-muted-foreground font-bold uppercase tracking-widest">No matching roles found</div>
                                 ...
                                 <div
                                    ...
                                    className="p-3 hover:bg-muted/50 rounded-lg text-sm text-foreground/80 font-bold cursor-pointer transition-colors flex items-center justify-between group"
                                 >
                                    {role}
               ...
               <div className="p-4 bg-white/[0.02] border border-border/25 rounded-xl space-y-2">
                  <div className="flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                     <Activity className="w-3.5 h-3.5" /> Data Feed Sync
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                     <div className="space-y-1">
                        <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider">Mission Prototype</span>
                        <div className="text-sm font-bold text-foreground/90">{team.name}</div>
                     </div>
                     <div className="space-y-1 text-right">
                        <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider">Target Objective</span>
                        <div className="text-sm font-bold text-muted-foreground">{team.event}</div>
                     </div>
                  </div>
               </div>
            ...
            {/* Footer Actions */}
            <div className="p-6 bg-white/[0.01] border-t border-border/25 flex items-center justify-end gap-3">
               <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-bold text-muted-foreground hover:bg-muted/50 hover:text-foreground/90 transition-all">
                  Cancel
               </button>
               <Button
